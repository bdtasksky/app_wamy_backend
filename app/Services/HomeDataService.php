...
class HomeDataService
{
    public function getHomePageData()
    {
        ...
        } elseif ($theme === 'storylane' || $theme === 'wordcraft') {
            $data += [
                'mostNewsCategory' => $this->getMostNewsCategory(),
            ];
        }
    ...
    private function retriveNewsDetail($slug)
    {
        $news = NewsMst::with(['category', 'subCategory', 'reporterBy', 'seoOnpage'])
                ->withCount('comments')
                ->with(['postByUser:id,full_name,profile_image', 'comments'])
    ...
    private function getDetailsRelatedNews($categoryId)
    {
        ...
        $archiveNews = NewsMst::with(['category', 'postByUser:id,full_name,profile_image', 'comments'])
            ->withCount('comments')
            ->where('language_id', $languageId)
            ->whereDate('publish_date', '<=', Carbon::today())
            ->where('status', 1)
    ...
    private function getHomePageTopNews()
    {
        ...
        $newsPositions = NewsPositionMap::with(['news' => function ($query) use ($languageId) {
                $query->select('id', 'news_id', 'encode_title', 'title', 'news', 'image', 'image_base_url', 'image_alt', 'image_title', 'post_by', 'publish_date', 'status', 'reader_hit')
                    ->withCount('comments')
                    ->where('language_id', $languageId)
                    ->whereDate('publish_date', '<=', Carbon::today())
                    ->where('status', 1)
    ...
    private function getSectionTwoNews()
    {
        ...
        if (activeTheme() == 'news' || activeTheme() == 'penmark') {
        ...
        if (activeTheme() == 'news') {
            ...
            $limitLeft = 5;
            $limitRight = 5;
        } elseif (activeTheme() == 'penmark') {
            $limitLeft = 8;
            $limitRight = 8;
        } elseif (activeTheme() == 'storylane') {
            $limitLeft = 8;
            $limitRight = 9;
        } elseif (activeTheme() == 'wordcraft') {
            $limitLeft = 16;
            $limitRight = 8;
        }
        ...
        if ($leftCategory) {
            $leftNews = NewsPositionMap::with(['news' => function ($query) use ($languageId) {
                    $query->select(
                        ...
                        'post_by', 'publish_date', 'status', 'reader_hit'
                    )
                    ->withCount('comments')
                    ->where('language_id', $languageId)
                    ->whereDate('publish_date', '<=', Carbon::today())
    ...
    private function getPopularNews()
    {
        ...
        // Manage news limit
        $limit = 5;
        if (activeTheme() == 'fashion' || activeTheme() == 'wordcraft') {
            $limit = 6;
        } elseif (activeTheme() == 'penmark') {
            $limit = 4;
        }
        ...
        $popularNews = NewsMst::select(columns: ['id', 'news_id', 'encode_title', 'title', 'news', 'image', 'image_base_url', 'image_alt', 'image_title', 'post_by', 'publish_date', 'status', 'category_id', 'reader_hit'])
            ->withCount('comments')
            ->with(['postByUser:id,full_name,profile_image', 'comments', 'category'])
            ->where('language_id', $languageId)
            ->whereDate('publish_date', '<=', Carbon::today())
            ->where('status', 1)
    ...
    private function getLatestNews()
    {
        ...
        $limit = 6;
        if (activeTheme() == 'penmark' || activeTheme() == 'storylane' || activeTheme() == 'wordcraft') {
            $limit = 8;
        }

        // Fetch the most last news
        $latestNews = NewsMst::select(columns: ['id', 'news_id', 'encode_title', 'title', 'news', 'image', 'image_base_url', 'image_alt', 'image_title', 'post_by', 'publish_date', 'status', 'created_at', 'category_id', 'reader_hit'])
            ->withCount('comments')
    ...
    private function getSectionThreeAllNews()
    {
        ...
        if (activeTheme() == 'news') {
            ...
            $limit = 5;
        } elseif (activeTheme() == 'times' || activeTheme() == 'storylane' || activeTheme() == 'wordcraft') {
            $limit = 8;
        } elseif (activeTheme() == 'penmark') {
            $position = 1;
            $limit = 5;
    ...
    private function getSectionFourNews()
    {
        ...
        if (activeTheme() == 'magazine') {
            ...
            $limit = 7;
        } elseif (activeTheme() == 'penmark' || activeTheme() == 'storylane' || activeTheme() == 'wordcraft') {
            $limit = 8;
        }
    ...
    private function getTrendingNews()
    {
        ...
        $topWeekNews = NewsMst::with(['postByUser:id,full_name,profile_image', 'comments'])
            ->select(['id', 'news_id', 'encode_title', 'category_id', 'title', 'news', 'image', 'image_base_url', 'image_alt', 'image_title', 'publish_date', 'post_by', 'reader_hit'])
            ->withCount('comments')
            ->where('language_id', $languageId)
            ->where('status', 1)
            ->whereDate('publish_date', '<=', Carbon::today())
    ...
    private function getSectionFiveNews()
    {
        ...
        if (activeTheme() == 'fashion') {
            $firstNewsLimit = 8;
        } elseif (activeTheme() == 'gazette' || activeTheme() == 'penmark' || activeTheme() == 'storylane' || activeTheme() == 'wordcraft') {
            $firstNewsLimit = 8;
            $secondNewsLimit = 8;
            $thirdNewsLimit = 8;
            $fourthNewsLimit = 8;
    ...
    private function getCommonSectionNews()
    {
        ...
        if (activeTheme() == 'fashion' || activeTheme() == 'times') {
           ...
           $limit = 5;
        } elseif (activeTheme() == 'gazette' || activeTheme() == 'penmark' || activeTheme() == 'storylane') {
            $limit = 8;
        }
    ...
    private function getCategoryNews($categoryId)
    {
        ...
        $popularNews = NewsMst::select(['id', 'news_id', 'encode_title', 'title', 'news', 'image', 'image_base_url', 'image_alt', 'image_title', 'post_by', 'publish_date', 'status', 'reader_hit'])
            ->withCount('comments')
            ->with(['postByUser:id,full_name,profile_image', 'comments'])
            ->where('category_id', $categoryId)
            ->where('language_id', $languageId)
    ...
    public function getSearchResults(string $searchTerm)
    {
        ...
        $newsResults = NewsMst::with(['postByUser:id,full_name,profile_image', 'comments', 'category'])
            ->withCount('comments')
            ->where('language_id', $languageId)
            ->whereDate('publish_date', '<=', Carbon::today())
            ->where('status', 1)
    ...
    public function getArchiveData($request)
    {
        ...
        $query = NewsArchive::query()
            ->select(['id', 'news_id', 'encode_title', 'title', 'news', 'image', 'image_base_url', 'image_alt', 'image_title', 'post_by', 'publish_date', 'status', 'reader_hit'])
            ->withCount('comments')
            ->where('language_id', $languageId)
            ->where('status', 1)
            ->whereDate('publish_date', '<=', Carbon::today());
    ...
    protected function getSlugFromUrl($string)
    {
        // If it's a URL, get the path basename
        if (filter_var($string, FILTER_VALIDATE_URL)) {
            $path = parse_url($string, PHP_URL_PATH);
            $string = basename($path);
        }

        // Remove query/hash if somehow still present
                ...
                try {
                    ...
                    $newsItems = collect($items)->take($feed->posts)->map(function ($item) use ($feed) {
                        ...
                        return [
                            ...
                            'encode_title'=> 'rss-details-' . $this->getSlugFromUrl($item->get_title()),
                            'link'        => $item->get_link(),
                            'guid'        => $item->get_id(),
                            'author'      => $dcCreator[0]['data'] ?? '',
                            'description' => $item->get_description(),
                            'pubDate'     => $item->get_date('Y-m-d H:i:s') ?: now()->format('Y-m-d H:i:s'),
    ...
    public function getMostNewsCategory()
    {
        $limit = null;
        if (activeTheme() == 'storylane') {
            $limit = 6;
        }

        $languageId = get_language_id();

        $query = Category::whereNull('parents_id')
            ->where('img_status', 0)
            ->where('language_id', $languageId)
            ->whereHas('news', function ($q) use ($languageId) {
                $q->where('status', 1)
                ->where('language_id', $languageId);
            })
            ->withCount(['news' => function ($q) use ($languageId) {
                $q->where('status', 1)
                ->where('language_id', $languageId);
            }])
    ...
    public function getTagNews(string $slug)
    {
        $languageId = get_language_id();

        // Get the tag first
        $topic = PostTag::select([
                'tag',
                'slug as encode_title',
                DB::raw("CONCAT('topic/', slug) as slug")
            ])
            ->where('slug', $slug)
            ->first();
        ...
        ->withCount('comments')
        ->with([
            'postByUser:id,full_name,profile_image',
            'comments'
        ])
        ->where('language_id', $languageId)
        ->whereDate('publish_date', '<=', Carbon::today())
        ->where('status', 1)
        ->whereHas('postTags', fn($q) => $q->where('slug', $slug))
        ->latest('created_at')
        ->take(21)
        ->get();
        ...
        return compact('topic', 'topicNews', 'popularNews', 'topWeekNews');
    }
