...
if (!function_exists('__url')) {
    function __url($path = '')
    {
        ...
        $permalinkType = Application::first()->permalink;

        $finalPath = $path;

        if ($permalinkType === 'category' || $permalinkType === 'date') {

            $news = NewsMst::with(['category', 'subCategory'])->where('encode_title', $path)->whereDate('publish_date', '<=', Carbon::today())->where('status', 1)->first();

            if ($news) {
                if ($permalinkType === 'category') {
                    if ($news->category && $news->subCategory) {
                        $categorySlug = $news->category->slug;
                        $subCategorySlug = $news->subCategory->slug;
                        $finalPath = "{$categorySlug}/{$subCategorySlug}/{$path}";
                    } else {
                        $categorySlug = $news->category->slug;
                        $finalPath = "{$categorySlug}/{$path}";
                    }
                } elseif ($permalinkType === 'date') {
                    $finalPath = permalink_date($news->publish_date)."/{$path}";
                }
            }

            $archive = NewsArchive::with(['category', 'subCategory'])->where('encode_title', $path)->first();

            if ($archive) {
                $pre_path = Str::slug(strtolower(localize('archive')));
                
                if ($permalinkType === 'category') {
                    if ($archive->category && $archive->subCategory) {
                        $categorySlug = $archive->category->slug;
                        $subCategorySlug = $archive->subCategory->slug;
                        $finalPath = "{$pre_path}/{$categorySlug}/{$subCategorySlug}/{$path}";
                    } else {
                        $categorySlug = $archive->category->slug;
                        $finalPath = "{$pre_path}/{$categorySlug}/{$path}";
                    }
                } elseif ($permalinkType === 'date') {
                    $finalPath = "{$pre_path}/".permalink_date($archive->publish_date)."/{$path}";
                }
            }

            $category = Category::with('get_parent_category')->where('slug', $path)->first();

            if ($category) {
                if ($category->get_parent_category) {
                    $parentCategorySlug = $category->get_parent_category->slug;
                    $finalPath = "{$parentCategorySlug}/{$path}";
                } else {
                    $finalPath = $path;
                }
            }

            $pagesData = Page::where('page_slug', $path)->first();

            if ($pagesData) {
                $pre_path = Str::slug(strtolower(localize('page')));
                
                if ($permalinkType === 'category') {
                    $finalPath = "{$pre_path}/{$path}";
                } elseif ($permalinkType === 'date') {
                    $finalPath = $path;
                }
            }

            $videoNewsData = VideoNews::where('encode_title', $path)->first();
            ...
            if ($videoNewsData) {
                ...
                if ($permalinkType === 'category') {
                    ...
                    $finalPath = "{$pre_path}/".permalink_date($videoNewsData->publish_date)."/{$path}";
                }
            }

            $opinion = Opinion::where('encode_title', $path)->first();
            ...
            if ($opinion) {
                ...
                if ($permalinkType === 'category') {
                    ...
                    $finalPath = "{$pre_path}/".permalink_date($opinion->updated_at)."/{$path}";
                }
            }
            ...
            $rssFeed = RssFeed::where('slug', $path)->first();
            ...
            if (Str::startsWith($path, 'rss-details-')) {
                $newsDetail = null;
                $feeds = RssFeed::all();
                $finalPath = $path;
                $pre_path = Str::slug(strtolower(localize('rss_feeds')));
                ...
                if ($newsDetail) {
                    ...
                    if ($permalinkType === 'category') {
                        $finalPath = "{$pre_path}/{$feed_name}/{$path}";
                    } elseif ($permalinkType === 'date') {
                        $finalPath = "{$pre_path}/".permalink_date($newsDetail['pubDate'])."/{$path}";
                    }
                }
            }
        }
        
        // Prefix language (only if not default)
        if ($lang === $defaultLang) {
            return url($finalPath);
        }

        return url($lang . '/' . ltrim($finalPath, '/'));
    }
}

if (!function_exists('permalink_date')) {
    /**
     * Format a given date into a permalink-friendly path (YYYY/MM/DD)
     *
     * @param  mixed  $date  Carbon|string|DateTime|null
     * @return string
     */
    function permalink_date($date)
    {
        if (empty($date)) {
            return '';
        }
...
if (!function_exists('get_advertisements')) {
    function get_advertisements(int $page, int $position)
    {
        ...
        return Advertisement::select('embed_code', 'status')
            ->with('themeRelation')
...
if (!function_exists('generateSlug')) {
    function generateSlug($title)
    {
        // 1. Convert to lowercase (supports Unicode languages)
        $slug = mb_strtolower($title, 'UTF-8');

        // 2. Remove punctuation but KEEP letters, numbers, spaces, hyphens & vowel signs
        $slug = preg_replace('/[^\p{L}\p{M}\p{N}\s-]+/u', '', $slug);

        // 3. Replace spaces or multiple hyphens with a single hyphen
        $slug = preg_replace('/[\s-]+/u', '-', trim($slug));
