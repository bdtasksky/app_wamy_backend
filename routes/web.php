...
Route::post('{lang}/load-more-category-news', [WebsiteHomeController::class, 'loadMoreCategoryNewsAjax'])->name('lang.load.more.category.news');
...
Route::post('{lang}/load-more-video-news', [WebsiteHomeController::class, 'loadMoreVideoNewsAjax'])->name('lang.load.more.video.news');
Route::post('/load-more-latest-posts', [WebsiteHomeController::class, 'loadMoreLatestPosts'])->name('load.more.latest.posts');
Route::post('{lang}/load-more-latest-posts', [WebsiteHomeController::class, 'loadMoreLatestPosts'])->name('load.more.latest.lang.posts');
Route::post('/load-more-topic-news', [WebsiteHomeController::class, 'loadMoreTopicNewsAjax'])->name('load.more.topic.news');
Route::post('{lang}/load-more-topic-news', [WebsiteHomeController::class, 'loadMoreTopicNewsAjax'])->name('lang.load.more.topic.news');
...
// Tags/Topic
Route::get('topic/{slug}', [WebsiteHomeController::class, 'tagToTopicNews'])
    ->where('slug', '[\p{L}\p{M}\p{N}_\-]+')
    ->name('tag.to.topic.news');
Route::get('{lang}/topic/{slug}', [WebsiteHomeController::class, 'tagToTopicNews'])
    ->where([
        'lang' => '[a-zA-Z]{2}',
        'slug' => '[\p{L}\p{M}\p{N}_\-]+',
    ])
    ->name('lang.tag.to.topic.news');

    
// Without language prefix
Route::get('/{param}', [WebsiteHomeController::class, 'resolvePermalink'])
    ->where('param', '^(?!admin)(.*)$')
    ->name('website.handle');

// With language prefix (e.g., /en/about)
Route::get('/{lang}/{param}', [WebsiteHomeController::class, 'resolvePermalink'])
    ->where([
        'lang' => '[a-zA-Z]{2}',
        'param' => '^(?!admin)(.*)$'
    ])
