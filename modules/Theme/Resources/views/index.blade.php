    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('themes.updateColors') }}" method="POST">
                ...
                @if ($activeTheme->name === 'Wordcraft')
                    <div class="row shadow-sm my-3 py-3">
                        <h6 class="fs-17 fw-semi-bold mb-0">{{ localize('hero_content') }}</h6>
                    </div>

                    <div class="form-group mb-3">
                        <label for="hero_title">{{ localize('hero_title') }}</label>
                        <input type="text" name="hero_title" id="hero_title" class="form-control" value="{{ $activeTheme->hero_title ?? localize('hero_title_content') }}" placeholder="{{ localize('hero_title') }}">
                    </div>

                    <div class="form-group mb-4">
                        <label for="hero_description">{{ localize('hero_description') }}</label>
                        <textarea name="hero_description" id="hero_description" class="form-control" placeholder="{{ localize('hero_description') }}">{{ $activeTheme->hero_description ?? localize('hero_description_content') }}</textarea>
                    </div>
                @endif
