    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function ($row) {
                ...
                if (auth()->user()->can('update_advertise') || auth()->user()->can('read_advertise')) {

                    if ($row->status === 1){
                        $status .= '<a href="javascript:void(0)" class="btn btn-sm btn-success advertise-status-update"
                            data-bs-toggle="tooltip" title="Update"
                            data-route="'.route('advertise.status.update', $row->id).'"
                            data-csrf="'.csrf_token().'"><i class="hvr-buzz-out far fa-check-circle"></i>
                        </a>';

                    }else{
                        $status .= '<a href="javascript:void(0)" class="btn btn-sm btn-danger advertise-status-update"
                            data-bs-toggle="tooltip" title="Update"
                            data-route="'.route('advertise.status.update', $row->id).'"
            ...
            ->rawColumns(['embed_code', 'ad_position','large_status','mobile_status','status','action']);
    }
