...
class AdvertisementController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:update_advertise')->only(['edit','update', 'updateLgStatus', 'updateSmStatus', 'updateStatus']);
        $this->middleware('permission:delete_advertise')->only('destroy');
        $this->middleware('demo')->only(['updateLgStatus','updateSmStatus','store', 'update', 'destroy', 'updateStatus']);
    }
     ...
     * @return Renderable
     */
    public function updateStatus(Advertisement $advertise)
    {
        $status = ($advertise->status==1?'0':'1');
