...
class SettingController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['demo'])->only(['deleteTaxGroup','activityLogDestroy', 'multipleDeleteActivityLog', 'cookieContentUpdate']);
    }
    ...
    public function cookieContent()
    {
        ...
        return view('setting::cookie_content', compact('cookieInfo'));
    }

    public function cookieContentUpdate(Request $request)
    {
        ...
        $cookieInfo = [
            ...
            'show_cookie' => $validated['show_cookie'] ?? 0,
        ];

        // Find the existing setting or create a new one
        $cookieSetting = Setting::firstOrNew(['event' => 'cookie_content']);
        $cookieSetting->details = json_encode($cookieInfo);
        $cookieSetting->save();

        return back()->with('success', localize('cookie_content_updated_successfully'));
    }

    public function googleRecaptcha()
    {
        $recaptchaData = Setting::select('details')->where('event','google_recaptcha')->first();
        $recaptchaInfo = json_decode($recaptchaData->details);

        return view('setting::google_recaptcha', compact('recaptchaInfo'));
    }
    ...
    public function googleRecaptchaUpdate(Request $request)
    {
        ...
        // Find the existing setting or create a new one
        $recaptchaSetting = Setting::firstOrNew(['event' => 'google_recaptcha']);
        $recaptchaSetting->details = json_encode($recaptchaInfo);
        $recaptchaSetting->save();

        return back()->with('success', localize('google_recaptcha_updated_successfully'));
    }
