...
class NewsService
{
    ...
    public function create(array $attributes): NewsMst
    {
        try {
            ...
            if ($auth_user->post_ap_status != 0) {
                ...
                $post_ap_status = $status = $attributes['status_confirmed'] ?? 0;
            }
            ...
            if (!empty($attributes['post_tag'])) {
                $tags = explode(',', $attributes['post_tag']);

                foreach ($tags as $tag) {
                    $cleanSlug = $this->explodedTitle($tag);
    ...
    public function update(array $attributes): NewsMst
    {
        ...
        try {
            ...
            if ($status == 1) {
                $newsMstData['time_stamp']   = time();
                $newsMstData['publish_date'] = date('Y-m-d');
            }
    ...
    private function explodedTitle(string $title)
    {
        ...
        $slug = preg_replace('/[^\p{L}\p{N}_\-]+/u', '-', strtolower(trim($title)));

        // Remove multiple hyphens
        $slug = preg_replace('/-+/', '-', $slug);

        // Trim any hyphens from start or end
        return trim($slug, '-');
    }
