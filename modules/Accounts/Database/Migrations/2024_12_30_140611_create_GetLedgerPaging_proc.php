...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE DEFINER=`remote`@`%` PROCEDURE `GetLedgerPaging`(IN `p_branch_Id` INT, IN `p_acc_coa_id` INT, IN `p_from_date` DATE, IN `p_to_date` DATE, IN `p_limit` INT, IN `p_page_number` INT, OUT `op_total_row` INT)
BEGIN
...
END");
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS GetLedgerPaging");
    }
};
