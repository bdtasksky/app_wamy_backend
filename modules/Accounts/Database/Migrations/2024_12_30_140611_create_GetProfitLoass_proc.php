...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE PROCEDURE `GetProfitLoass`(IN `p_branch_Id` INT, IN `p_from_date` DATE, IN `p_to_date` DATE)
BEGIN
    ...
    CLOSE cur;
        ...
        (SELECT COALESCE(SUM(debit), 0) FROM temp_table_pl WHERE nature_id = t.nature_id) AS nature_amount_debit,
        (SELECT COALESCE(SUM(credit), 0) FROM temp_table_pl WHERE nature_id = t.nature_id) AS nature_amount_credit,
         
	t.group_id,
	g.account_name AS group_name,
        (SELECT COALESCE(SUM(debit), 0) FROM temp_table_pl WHERE group_id = t.group_id) AS group_amount_debit,
        (SELECT COALESCE(SUM(credit), 0) FROM temp_table_pl WHERE group_id = t.group_id) AS group_amount_credit,
	  
	t.sub_group_id,
        sg.account_name AS sub_group_name,
        (SELECT COALESCE(SUM(debit), 0) FROM temp_table_pl WHERE sub_group_id = t.sub_group_id) AS sub_group_amount_debit,
        (SELECT COALESCE(SUM(credit), 0) FROM temp_table_pl WHERE sub_group_id = t.sub_group_id) AS sub_group_amount_credit,
	 
        t.ledger_id,
        t.acc_name AS ledger_name,
        t.debit,
        t.credit
        FROM temp_table_pl t
...
END");
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS GetProfitLoass");
    }
};
