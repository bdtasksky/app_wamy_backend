...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE PROCEDURE `GetLedger_OCB`(IN `p_branch_Id` INT, IN `p_acc_coa_id` INT, IN `p_from_date` DATE, IN `p_to_date` DATE, OUT `p_opening_balance` DOUBLE(15,2), OUT `p_closing_balance` DOUBLE(15,2), OUT `p_trangection_balance` DOUBLE(15,2))
BEGIN
    ...
    DECLARE l_financial_year_id INT;
    DECLARE vh_id INT;
  ...
  DECLARE cur_py CURSOR FOR SELECT voucher_master_id, VoucherNumber, account_name, Remarks, VoucherDate, Dr_Amount, Cr_Amount  FROM temp_cursor_results;
    ...
    LIMIT 1;
	   ...
	   IF l_current_fy_status=2 THEN
		...
		'INSERT INTO temp_cursor_results (voucher_master_id, VoucherNumber, account_name, Remarks, VoucherDate, Dr_Amount, Cr_Amount) ',
...
END");
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS GetLedger_OCB");
    }
};
