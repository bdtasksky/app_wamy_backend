...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE DEFINER=`remote`@`%` PROCEDURE `GetLedgerSubCodePaging`(IN `p_branch_Id` INT, IN `p_acc_coa_id` INT, IN `subCode_id` INT, IN `p_from_date` DATE, IN `p_to_date` DATE, IN `p_limit` INT, IN `p_page_number` INT, OUT `op_total_row` INT)
BEGIN
...
IF l_current_fy_status=1 THEN
    ...
    read_loop: LOOP
        ...
        INSERT INTO temp_table (v_date, v_voucher_no, v_rev_acc_name, v_remarks, v_debit, v_credit, v_balance, v_voucher_id, v_voucher_type_id)
        VALUES (t_v_date, vh_v_no, l_rev_acc_name, vh_remarks , IFNULL(t_debit, 0.00), IFNULL(t_credit, 0.00), opening_balance, vh_id, vh_acc_voucher_type_id);
    END LOOP;
...
END");
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS GetLedgerSubCodePaging");
    }
};
