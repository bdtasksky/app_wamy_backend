...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE PROCEDURE `GetTrialFullBalance`(IN `p_branch_Id` INT, IN `p_from_date` DATE, IN `p_to_date` DATE)
BEGIN
...
CALL GetLedger_OCB(p_branch_Id,l_acc_id,p_from_date, p_to_date, @p_opening_balance, @p_closing_balance, @p_trangection_balance);
    ...
    SELECT 
        ...
        (SELECT COALESCE(SUM(debit), 0) FROM temp_table_tb WHERE nature_id = t.nature_id) AS nature_amount_debit,
        (SELECT COALESCE(SUM(credit), 0) FROM temp_table_tb WHERE nature_id = t.nature_id) AS nature_amount_credit,
         
	t.group_id,
	g.account_name AS group_name,
        (SELECT COALESCE(SUM(debit), 0) FROM temp_table_tb WHERE group_id = t.group_id) AS group_amount_debit,
        (SELECT COALESCE(SUM(credit), 0) FROM temp_table_tb WHERE group_id = t.group_id) AS group_amount_credit,
	  
	t.sub_group_id,
        sg.account_name AS sub_group_name,
        (SELECT COALESCE(SUM(debit), 0) FROM temp_table_tb WHERE sub_group_id = t.sub_group_id) AS sub_group_amount_debit,
        (SELECT COALESCE(SUM(credit), 0) FROM temp_table_tb WHERE sub_group_id = t.sub_group_id) AS sub_group_amount_credit,
	 
        t.ledger_id,
        t.acc_name AS ledger_name,
        t.debit,
        t.credit
        FROM temp_table_tb t
...
END");
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS GetTrialFullBalance");
    }
};
