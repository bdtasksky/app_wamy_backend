        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title">
                    <h4>{{ __('language.balance_sheet') }}
                        {{-- <div class="btn-group pull-right form-inline">
                            <button id="hideUnhideButton" type="button" class="btn btn-success">{{ __('language.Hide/Unhide Zero Value Rows') }}</button>
                        </div> --}}
                    </h4>
                </div>
            </div>
                ...
                <form action="{{ route('account.report.balance.sheet.report.search') }}" method="POST" class="form-inline">
                    ...
                    <div class="row">
                        <div class="col-sm-2 form-group form-group-new empdropdown">
                            ...
                            <select id="financial_year" class="form-control" name="dtpYear">
                                ...
                                @foreach ($financial_years as $year)
                                    <option value="{{ $year->title }}" data-start_date="{{ $year->start_date }}" data-end_date="{{ $year->end_date }}"
                        ...
                        <div class="col-sm-2 form-group form-group-new">
                            ...
                            <input type="text" name="dtpFromDate" value="{{ $dtpFromDate ?? '' }}" class="form-control" id="from_date" />
                        </div>

                        <div class="col-sm-2 form-group form-group-new">
                            <label for="dtpToDate">{{ __('language.to_date') }} :</label>
                            <input type="text" class="form-control" name="dtpToDate" value="{{ $dtpToDate ?? '' }}" id="to_date" />
                        </div>
                        ...
                        <div class="col-sm-2">
                            <button style="margin-top:9%" type="submit" class="btn btn-success">{{ __('language.search') }}</button>
                            <button style="margin-top:9%" type="button" class="btn btn-success" id="hideUnhideButton">{{ __('language.hide_zero_balance') }}</button>
                        </div>
                    </div>
...
<div class="card mt-5 px-3 py-5 col-md-12">
    @php
    ...
    <div id="printArea">
        ...
        <div class="table-responsive">
            <table width="100%" class="table table-striped table-bordered table-hover balance-sheet-report" id="balance-sheet-report">
                <thead>
                    <tr class="header-row">
                            ...
                            @elseif ($asset->level == 4)
                                <tr class="voucherList">
                                    <td style="padding-left:120px;">
                                        <a  href="{{route('account.report.general.ledger.by-link',['dtpYear'=>$dtpYear,'cmbCode'=>$asset->id,'dtpFromDate'=>$dtpFromDate,'dtpToDate'=>$dtpToDate])}}" target="_blank" class=""> {{ $asset->name }}</a>
                                    </td>
                    ...
                    @isset($getBalanceSheetLiabilities)
                        @foreach ($getBalanceSheetLiabilities as $liability)
                            ...
                            @elseif ($liability->level == 4)
                                <tr class="voucherList">
                                    <td style="padding-left:120px;">
                                        <a href="{{route('account.report.general.ledger.by-link',['dtpYear'=>$dtpYear,'cmbCode'=>$liability->id,'dtpFromDate'=>$dtpFromDate,'dtpToDate'=>$dtpToDate])}}" target="_blank" class=""> {{ $liability->name }}</a>
                                    </td>
