...
@section('title', __('language.deferred_schedule'))
@push('css')
@endpush
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h6 class="fs-17 fw-semi-bold mb-0">{{ __('language.deferred_schedule') }}</h6>
                </div>
                <div class="text-end">
                    <div class="actions">
                ...
                <table width="100%" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody id="ledger-body">
                        <tr class="text-center">
                            <td colspan="8">{{ __('language.no_data_found') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
@push('js')
    <script>
        window.appData = {
            getListUrl: "{{ route('accounts.voucher.getDeferredSchedule') }}",
            voucherDetailsUrl: "{{ route('accounts.voucher.details') }}",
        };
    </script>
    <script src="{{ asset('public/backend/assets/sweetalert.js') }}"></script>
    <script src="{{ module_asset('Accounts/js/deferred-schedule.js?v=' . time()) }}"></script> <!-- Your custom script -->
    <script>
