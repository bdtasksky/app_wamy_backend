    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response.coaDropDown, function(key, value) {
                coaOptions += "<option value='" + value.id + "'>" + value.account_name + "</option>";
            });
            $('#acc_coa_id').append(coaOptions);

            // Handle Subcode Dropdown
            var subcodeOptions = "<option value='0'>Select One</option>"; // Default option for Subcode
            $.each(response.subcode, function(key, value) {
                subcodeOptions += "<option value='" + value.id + "'>" + value.name + "</option>";
            });
