            ...
            if (!transactions || transactions.length === 0) {
                ...
                $.each(transactions, function (index, transaction) { 
                    ...
                    rowsHtml += '<a href="javascript:" data-id="' + transaction.voucher_master_id + '" data-vdate="' + transaction.VoucherDate + '" '
                        ...
                        // *** NEW CODE: Add Schedule Status button if IsDeferred ***
                        if (transaction.ScheduleStatus === 'IsDeferred') {
                            rowsHtml += '<button class="btn btn-xs btn-warning schedule-status-btn" style="margin-right:10px" '
                                    + 'data-id="' + transaction.voucher_master_id + '" '
                                    + 'data-voucher-no="' + transaction.VoucherNumber + '" '
                                    + 'data-voucher-date="' + transaction.VoucherDate + '" '
                                    + 'data-total-amount="' + transaction.TranAmount + '" '
                                    + 'data-voucher-status="' + transaction.IsApprove + '" '
                                    + 'title="Schedule Status">'
                                    + '<i class="fa fa-calendar"></i></button>';

                            rowsHtml += '<button class="btn btn-xs btn-danger remove-deferred-btn" style="margin-right:10px" '
                                    + 'data-id="' + transaction.voucher_master_id + '" '
                                    + 'title="Remove Deferred Schedule">'
...
$(document).ready(function() {
    ...
    $('#ledger-body').on('click', '.schedule-status-btn', function() {
        ...
        const voucherStatus = $(this).data('voucher-status');
        ...
        if(voucherStatus==0){
            $('#modalVoucherStatus').removeClass('d-none');
            $('#saveDeferredSchedule').addClass('d-none');
        }else{
            $('#modalVoucherStatus').addClass('d-none');
            $('#saveDeferredSchedule').removeClass('d-none');
        }
    });
...
function getVoucherAmount(master_id){
    var voucherBalanceUrl = window.appData.voucherBalanceUrl; 
    if (!master_id) {
    $('#balance_amount').text('0.00');
    return;
    }
    $.ajax({
    url: voucherBalanceUrl.replace(':id', master_id),
    type: "GET",
    dataType: "json",
      success: function(response) {
        if (response.success && response.balance !== undefined) {
            $('#modalTotalAmountText').text(response.balance);
            $('#modalTotalAmount').val(response.balance);
        } else {
            $('#modalTotalAmountText').text(0.00);
            $('#modalTotalAmount').val(0.00);
        }
    },
    error: function() {
         $('#modalTotalAmountText').text(0.00);
...
$('#scheduleStatusModal').on('shown.bs.modal', function () {
    // Initialize select2 inside the modal
    $('#expense_head').select2({
        dropdownParent: $('#scheduleStatusModal'),
