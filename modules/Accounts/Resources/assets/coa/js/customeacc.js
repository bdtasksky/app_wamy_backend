...
$(".approved_voucher").click(function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // Disable the button here or perform any other actions
...
$(document).ready(function () {
    // Your existing code to check and hide rows
    function checkAndHideRows() {
        $('#trail-balance-table tbody tr').each(function () {

            var columnsToCheck = [2, 3, 4, 5, 6, 7]; // Columns 3, 4, 5, 6, 7, and 8 (0-based index)
            var hideRow = true;
            ...
            for (var i = 0; i < columnsToCheck.length; i++) {
                ...
                if (value !== 0) {
                    hideRow = false;
                    break; // No need to check other columns if one of them is non-zero
                }
            }
            ...
            if (hideRow) {
                ...
                $(this).show(); // Ensure that if previously hidden, it's shown when conditions are not met
            }
        });
    }
    ...
    $('#hideUnhideButton').on('click', function () {
        if(hide){
            $('#trail-balance-table tbody tr').toggle(); // Toggle visibility of all rows
            
        }else{
            checkAndHideRows();
        }
        hide = true // Check and hide rows again based on the conditions
    });
});

"user strict";
function voucherDetailsView(id) {
    var url = $('#voucherDetailsView-'+id).data('url');
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data){
                $('#viewData').html('');
                $('#viewData').html(data);
                $('#voucherDetailsViewModal').modal('show');
            }
        },
        error: function (data) {
