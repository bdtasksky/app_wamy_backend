...
class SeoController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:create_custom_code')->only('customCodeStore');
        $this->middleware('permission:update_custom_code')->only('customCodeStore');

        $this->middleware(['demo'])->only(['store','socialSiteStore','socialLinkStore','customCodeStore']);
    }
    ...
    /**
     * Summary of customCode
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
     */
    public function customCode()
    {
        $data = Setting::where('event','custom_code')->first();
        $custom_code = json_decode($data->details);

        return view('seo::custom_code', compact('custom_code'));
    }

    /**
     * Summary of customCodeStore
     * @param \Illuminate\Http\Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function customCodeStore(Request $request)
    {
        try {
            $details_data['tags']   = $request->input("tags") ?? '';
            $details_data['status'] = $request->input("status") ?? 0;
