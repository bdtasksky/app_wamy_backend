...
class CategoryController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($slug != '' && $slug != $category->slug) {
            $check_data_exist = $this->check_category_existence($slug, $category->id);
            // if it is exist already
            if ($check_data_exist) {
                // You can customize the error message based on the type of exception
                return response()->json(['error' => true, 'msg' => 'Category already exists !']);
    ...
    public function check_category_existence($slug, $ignoreId = null)
    {
        $languageId = get_language_id();

        $query = Category::where('slug', $slug)->where('language_id', $languageId);
