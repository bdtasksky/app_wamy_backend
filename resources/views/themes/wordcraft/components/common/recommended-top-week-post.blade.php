@if ($topWeekNews->isNotEmpty())
    <section 
        class="my-4 space-y-3 md:border md:p-2 md:rounded-md bg-white dark:bg-neutral-900 dark:divide-neutral-700 dark:border-neutral-800 shadow-sm 2xl:p-3 p-2">
        <h2 class="@if (!$themeSettings->background_color || mode()) bg-neutral-700 @endif @if (!$themeSettings->text_color || mode()) text-white @endif p-2 uppercase mb-2"
        ...
        <a href="{{ __url($topWeekNews[0]->encode_title) }}" class="bg_gradient_top block h-[289px]">
            ...
            <div class="p-2 2xl:p-3 absolute z-10 bottom-0 left-0">
                ...
                <h1 class="text-white hover:text-sky-600 font-bold text-xl my-2">
                    {{ $topWeekNews[0]->title }}
                </h1>
            ...
            @foreach ($topWeekNews->slice(1) as $topWeekNewsItem)
                <div class="grid grid-cols-3 gap-2 items-center">
                    ...
                    <a href="{{ __url($topWeekNewsItem->encode_title) }}"
                        class="col-span-2 line-clamp-3 transition_3 hover:text-sky-600 dark:hover:text-sky-600 dark:text-neutral-50">
