...
<body class="min-h-screen relative" @if (!$themeSettings->is_default &&
	$themeSettings->font_family) style="font-family: {{ $themeSettings->font_family }};" @endif>

	@include('themes.wordcraft.components.common.login')
	{{-- Side Menu section --}}
	@include('themes.wordcraft.layouts.header.side-menu')

	<!-- Top Section -->
	@include('themes.wordcraft.layouts.header.top-section')

	<div class="mt-16">
		@include ('themes.wordcraft.layouts.header.mobile-menu')
		{{ $slot }}
	</div>


	<!-- Footer section -->
	@include('themes.wordcraft.layouts.footer')

	<!-- Cookies section -->
	@if ($cookieInfo->show_cookie == 1)
		@include('themes.wordcraft.components.common.cookies')
	@endif
