<footer class="relative w-full h-max isolate @if (!$themeSettings->footer_color || mode()) bg-neutral-800 @endif"
    @if (!mode() && $themeSettings->footer_color) style="background-color: {{ $themeSettings->footer_color }};" @endif>
    <div class="container py-8">
        @include ('themes.wordcraft.components.common.footer-heading')

        <section
            class="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4 xl:gap-0 border-y border-neutral-700 py-8 lg:divide-x lg:rtl:divide-x-reverse lg:divide-neutral-700">
            <!--  {/* Address Section */} -->
            ...
            <!-- {/* Get connected */} -->
            <div class="text-white lg:pl-4 2xl:pl-6 rtl:lg:pr-4 rtl:2xl:pr-6">
                <h2 class="text-xl font-bold mb-8">{{ localize('sign_up_for_our_newsletter') }}</h2>
                ...
                <form id="subscribe-form" class="relative my-6 max-w-xl mx-auto">
                    ...
                    <div class="relative">
                        <input name="subscriber_email" id="subscriber_email"
                            class="text-neutral-800 py-2.5 pl-3 pr-28 rounded-md w-full duration-300 ease-in placeholder:text-sm"
                            type="email" placeholder="Email" required />

                        <button type="submit" id="subscribe-btn"
                            class="absolute top-1/2 -translate-y-1/2 right-0 rounded-r-md bg-sky-700 text-white px-4 h-full flex items-center justify-center gap-1 transition-all duration-300 ease-in">
                            {{ localize('subscribe') }}
                        </button>
                    </div>
                </form>
