<x-web-layout :seoSchemasInfo="$newsDetail">
    <!-- Pagination -->
    ...
    <section class="container mt-2 pb-8 grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 4xl:grid-cols-4 gap-4">
        ...
        <section class="lg:col-span-2 xl:col-span-2 4xl:col-span-3 gap-6">
            ...
            <div class="p-8 space-y-6">
                <h1 class="dark:text-white text-2xl lg:text-3xl font-semibold line-clamp-2">
                    {{ $newsDetail->title }}
                </h1>
                @if ($newsDetail->stitle)
                    <p class="line-clamp-2 text-neutral-500 dark:text-neutral-50">
                        {{ $newsDetail->stitle }}
                    </p>
                @endif

                <div class="flex md:items-center justify-between flex-col md:flex-row gap-4 mt-4">
                    <div class="text-neutral-500 dark:text-neutral-50 capitalize flex items-center gap-2 text-sm">
                        <figure class="w-10 h-10 overflow-hidden rounded-full">
                            <img class="w-full h-full object-cover"
                                src="{{ isset($newsDetail->postByUser->profile_image) ? asset('storage/' . $newsDetail->postByUser->profile_image) : asset('/assets/profile.png') }}"
                                alt="{{ $newsDetail->postByUser->full_name ?? localize('unknown') }}" />
                        </figure>
                        ...
                        <span>{{ news_publish_date_format($newsDetail->publish_date) }}</span>
                    ...
                    <!-- Social section -->
                    @include ('themes.storylane.components.common.social-section')
                </div>
            </div>
            ...
            <div class="dark:text-white">
                ...
                {{-- Post Tag --}}
                @if (optional($newsDetail->postTags)->isNotEmpty())
                    <div class="bg-white rounded-md shadow-sm p-3 mt-4 border border-gray-200">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($newsDetail->postTags as $postTag)
                                @php 
                                    $tagUrl = __url("topic/{$postTag->slug}");
                                @endphp

                                <a href="{{ $tagUrl }}" 
                                class="inline-block text-neutral-700 bg-gray-100 px-3 py-1 rounded capitalize hover:text-brand-primary transition_3"
                                aria-label="{{ localize('view_all_posts_tagged').' '.$postTag->tag }}">
                                    {{ $postTag->tag }}
                                </a>
                            @endforeach
                        </div>
        ...
        <section>
            <div class="space-y-6 sticky top-16">
                ...
                <!-- Ads section -->
                <figure class="">
                    @php $ad = get_advertisements(3, 1); @endphp
    ...
    <section class="container mt-2 pb-8 grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 4xl:grid-cols-4 gap-4">
        ...
        <section class="lg:col-span-2 xl:col-span-2 4xl:col-span-3 gap-6">
            ...
            <!-- add section -->
            <figure class="mt-6">
                @php $ad = get_advertisements(3, 2); @endphp
            ...
            <!-- Article-slider -->
            @include ('themes.storylane.components.slider.article-slider')
            ...
            @if ($sectionSixNews->isNotEmpty())
                @include ('themes.storylane.components.common.related-post')
            @endif
            ...
            @if (app_setting()->web_user_can_comment == 1)
                @include ('themes.storylane.components.common.comment-section')
            @endif
        ...
        <section class="md:w-1/2 lg:w-auto">
            <div class="space-y-6 sticky top-16">
                <!-- Top Week -->
                @include('themes.storylane.components.common.recommended-top-week-post')

                <!-- Voting poll -->
                @if ($votingPoll)
                    @include('themes.storylane.components.common.voting-poll')
                @endif

                <!-- Ads section -->
                <figure class="">
                    @php $ad = get_advertisements(3, 3); @endphp
