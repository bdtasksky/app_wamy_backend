<!-- Top Section -->
<section
    class="@if (!$themeSettings->background_color || mode()) bg-white dark:bg-black   dark:border-neutral-800 @endif top_header border-b transition-all duration-500 z-20 ease-in dark:shadow-white/10 fixed left-0 top-0 w-full"
    ...
    <div class="container mx-auto py-2 flex justify-between items-center">
        ...
        <button onclick="openSidebar()" type="button"
            class="xl:hidden bg-neutral-100 dark:bg-theme-six-secondary text-neutral-600 dark:text-neutral-50 rounded-full w-9 h-9 flex justify-center items-center">
        ...
        <!-- Brand Logo -->
        <div class="">
            <a class="w-32 h-6 flex_center overflow-hidden dark:hidden" href="{{ __url('/') }}">
                <img src="{{ app_setting()->logo }}" alt="Brand logo light" />
            </a>
            <a class="w-32 h-6 flex_center overflow-hidden hidden dark:block" href="{{ __url('/') }}">
                <img src="{{ app_setting()->footer_logo }}" alt="Brand logo dark" />
            </a>
        </div>

        @include('themes.storylane.layouts.header.menu')

        <!-- Right section  -->
        <section class="flex items-center gap-2 xl:gap-2">
            <!-- Multi-language Start -->
            @if ($languages->count() > 1)
                <div class="hidden xl:block relative text-left">
                    <button type="button" id="lang-toggle"
                        class="text-sm bg-white border border-gray-300 rounded-lg px-2 py-1 shadow-sm hover:bg-gray-50 focus:outline-none flex gap-0.5 dark:bg-theme-six-secondary dark:border-theme-six-secondary dark:text-neutral-50">
            ...
            <!-- Login / Registration -->
            @include('themes.storylane.components.common.login-button')

            <!-- Dark/Light Switch-->
            <button id="theme-toggle" type="button" aria-label="Use Dark Mode"
                class="btn_theme bg-neutral-100 dark:bg-theme-six-secondary text-orange-600 dark:text-white w-9 h-9 flex justify-center items-center rounded-full border dark:border-neutral-800">
            ...
            <!-- Search button -->
            <div class="xl:hidden">
                @include('themes.storylane.components.common.search-button')
            </div>
