...
<body class="min-h-screen relative"
    ...
    @include('themes.gazette.components.common.login')

    {{-- Menu section --}}
    @include('themes.gazette.layouts.header.side-menu')

    @include('themes.gazette.layouts.header.top-menu')

    <!-- Top Section -->
    @include('themes.gazette.layouts.header.top-section')

    @include ('themes.gazette.layouts.header.mobile-menu')

    {{ $slot }}

    <!-- Footer section -->
    @include('themes.gazette.layouts.footer')

    <!-- Cookies section -->
    @if ($cookieInfo->show_cookie == 1)
	    @include('themes.gazette.components.common.cookies')
    @endif
    ...
    <button id="scrollToTopBtn" type="button" onclick="window.scrollTo({ top: 0, behavior: 'smooth' })"
        class="hidden bg-theme-five-primary hover:bg-blue-800 rounded-full w-10 h-10 flex justify-center items-center fixed bottom-4 right-2 hover:-translate-y-1 z-50 transition-all duration-300 ease-in">
