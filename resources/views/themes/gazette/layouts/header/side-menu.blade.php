...
<div id="mobileMenuModal" class="fixed inset-0 z-50 bg-black bg-opacity-60 hidden">
    <!-- Sidebar -->
    <div class="side-bar bg-theme-five-secondary h-full p-6 pt-2 overflow-x-hidden overflow-y-auto shadow-md side_bar"
        id="sidebar">
        <!-- Close Button -->
        ...
        <!-- Login / Registration -->
        @include('themes.gazette.components.common.login-button-sm')

        <!-- Your existing sidebar menu goes here -->
        <div class="menu w-full space-y-4 mt-6">
            <div class="item relative cursor-pointer">
                <a class="capitalize text-white flex justify-start items-center text-base hover:transition_3 side_nav_gazette"
            ...
            @foreach ($mainMenus as $menu)
                @if ($menu->submenus->isNotEmpty())
                    <div class="item relative cursor-pointer">
                        <a
                            class="sub-btn text-white flex justify-between items-center text-base hover:transition_3 hover:text-red-600 active:text-red-600 side_nav_gazette">{{ Str::upper($menu->menu_level) }}
                            ...
                            @foreach ($menu->submenus as $submenu)
                                @if ($loop->iteration == 1)
                                    <a href="{{ __url($menu->slug) }}"
                                        class="sub-item my-1.5 ml-7 mr-1.5 pt-2 pb-1 text-white flex justify-start items-center text-base hover:transition_3 relative hover:text-red-500 active:text-red-500 side_nav_gazette">{{ localize('all') }}</a>
                                @endif
                                <a href="{{ __url($submenu->slug) }}"
                                    class="sub-item my-1.5 ml-7 mr-1.5 pt-2 pb-1 text-white flex justify-start items-center text-base hover:transition_3 relative hover:text-red-500 active:text-red-500 side_nav_gazette">{{ $submenu->menu_level }}</a>
                            @endforeach
                        </div>
                    </div>
                @else
        ...
        @if ($languages->count() > 1)
            <div class="relative inline-block mt-6">
                <button type="button" id="dropdownButton"
                    class="text-white bg-blue-800 transition_3 px-4 py-1.5 rounded-md">
                ...
                <ul id="dropdownMenu"
                    ...
                    @foreach ($languages as $lang)
                        <li>
                            <button
                                class="lang-option block w-full text-left px-4 py-2 hover:bg-gray-500 hover:dark:bg-neutral-800 rtl:text-right {{ $lang->value == app()->getLocale() ? 'bg-neutral-700 text-white' : 'text-white' }}"
