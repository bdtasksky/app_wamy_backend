...
<body class="min-h-screen relative">
    @if (!request()->is('gallery'))
        @include('themes.times.components.common.login')
        {{-- Side Menu section --}}
        @include('themes.times.layouts.header.side-menu')

        <!-- Top Section -->
        @include('themes.times.layouts.header.top-section')
        {{-- Menu section --}}
        @include('themes.times.layouts.header.top-menu')
        @include ('themes.times.layouts.header.mobile-menu')
    @endif
    ...
    <!-- Footer section -->
    @if (!request()->is('gallery'))
        @include('themes.times.layouts.footer')
    @endif

    <!-- Cookies section -->
    @if ($cookieInfo->show_cookie == 1)
	    @include('themes.times.components.common.cookies')
    @endif
    ...
    <button id="scrollToTopBtn" type="button" onclick="window.scrollTo({ top: 0, behavior: 'smooth' })"
        class="hidden bg-blue-600 hover:bg-blue-500 rounded-full w-10 h-10 flex justify-center items-center fixed bottom-4 right-2 hover:-translate-y-1 z-50 transition-all duration-300 ease-in">
    ...
    <script>
        ...
        const categoryLoadMoreUrl = "{{ __url('load-more-category-news') }}";
        const topicLoadMoreUrl = "{{ __url('load-more-topic-news') }}";
        const videoLoadMoreUrl = "{{ __url('load-more-video-news') }}";
    ...
    <script src="{{ \App\Helpers\ThemeHelper::asset('js/swiper/opinion-slider.js') }}"></script>
